...
void Object::getVert(sf::VertexArray &vert){
...
void Object::getText(sf::Text &newText){
	if (mode == 0){
		newText = text;
	} else if (mode == 1){
		newText = hover_text;
...
void Object::animate(float ecl){
	ecl++;
	//nothing
}

void Object::hover(const sf::Vector2i mouse){
...
bool Object::click(const sf::Vector2i mouse){
	if(mouse.x > position.x && mouse.x < position.x+size.x && mouse.y > position.y && mouse.y < position.y+size.y){
		mode = 2;
		return 1;
	}
...
void Object::addChar(const char newChar){
...
void Object::setTexturePosition(const sf::Vector2f new_texture_position){
...
void Object::setTextureSize(const sf::Vector2f new_texture_size){
...
void Object::setSize(const sf::Vector2f new_size){
	size = new_size;
	hover_size = new_size;
	click_size = new_size;
}

void Object::setColor(const sf::Color new_color){
	color = new_color;
	hover_color = new_color;
	click_color = new_color;
}

void Object::setBorderWidth(const int newWidth){
	border_width = newWidth;
	hover_border_width = border_width;
	click_border_width = border_width;
}

void Object::setBorderColor(const sf::Color newColor){
	border_color = newColor;
	hover_border_color = border_color;
	click_border_color = border_color;
}

void Object::setText(const sf::Text &new_text){
	text = new_text;
	hover_text = new_text;
	click_text = new_text;
}

void Object::setTextColor(const sf::Color newColor){
	text.setColor(newColor);
	hover_text.setColor(newColor);
	click_text.setColor(newColor);
}

void Object::alignText(){
	text.setOrigin(0,0);
...
void Object::setFont(const sf::Font &new_font){
	font = new_font;
}

sf::Font Object::getFont(){
	return font;
}
